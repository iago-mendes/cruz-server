	...
	show: async (req: Request, res: Response) =>
	{
		...
		await Promise.all(goal.companies.map(async company =>
		{
			...
			await Promise.all(company.sellers.map(async seller =>
			{
				...
				const requests = await RequestModel.find({representada: company.id, vendedor: seller.id})
			...
			const eCommerceRequests = await RequestModel.find({representada: company.id, vendedor: undefined})
