								...
								if (line !== undefined)
								{
										...
										const promises = request.produtos.map(async productSold =>
										{
												line.produtos.map(product =>
												{
														...
														{
																const clientCompany = client.representadas
																	.find(tmpCompany => tmpCompany.id == company._id)
																if (!clientCompany)
																	return res.json({message: 'client company not found'})

																const tableId = clientCompany.id

																const clientProduct = line.produtos
																	.find(tmpProduct => tmpProduct._id == product._id)
																if (!clientProduct)
																	return res.json({message: 'client product not found'})

																const table = clientProduct.tabelas
																	.find(tmpTable => tmpTable.id == tableId)
																if (!table)
